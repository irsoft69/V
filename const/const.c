...
#define V_CURRENT_COMMIT_HASH "7e4e3ab"
#endif
...
static DenseArray new_dense_array();
static u32 DenseArray_push(DenseArray* d, KeyValue kv);
static voidptr DenseArray_get(DenseArray d, int i);
static void DenseArray_zeros_to_end(DenseArray* d);
static map new_map(int n, int value_bytes);
...
int utf8_getchar();
static string Color_str(Color c);
static Color rgb(int r, int g, int b);
array_int _const_numbers; // inited later
Color _const_red; // inited later
Color _const_blue; // inited later
// variadic structs
struct varg_voidptr {
	int len;
	voidptr args[0];
...
string array_int_str(array_int a) {
	...
	for (int i = 0; i < a.len; i++) {
		...
		strings__Builder_write(&sb, int_str(it));
		if (i < a.len-1) {
			strings__Builder_write(&sb, tos3(", "));
		}
	}
...
array array_repeat(array a, int count) {
	if (count < 0) {
		v_panic(_STR("array.repeat: count is negative: %"PRId32"\000", 2, count));
	}
...
static void panic_debug(int line_no, string file, string mod, string fn_name, string s) {
	...
	println(_STR("   module: %.*s\000", 2, mod));
	println(_STR(" function: %.*s\000()", 2, fn_name));
	println(_STR("     file: %.*s\000", 2, file));
	println(string_add(tos3("     line: "), int_str(line_no)));
	println(_STR("  message: %.*s\000", 2, s));
	println(tos3("========================================="));
...
void v_panic(string s) {
	println(_STR("V panic: %.*s\000", 2, s));
	print_backtrace();
	exit(1);
}
...
static voidptr __as_cast(voidptr obj, int obj_type, int expected_type) {
	if (obj_type != expected_type) {
		v_panic(_STR("as cast: cannot cast %"PRId32"\000 to %"PRId32"\000", 3, obj_type, expected_type));
	}
	return obj;
}
...
// $if  glibc {
			...
			for (int tmp4 = 0; tmp4 < tmp3.len; tmp4++) {
				...
				string cmd = _STR("addr2line -e %.*s\000 %.*s\000", 3, executable, addr);
				...
				println(_STR("%*.*s\000 | %*.*s\000 | %.*s\000", 4, output, -46, addr, 14, beforeaddr));
			}
			return true;
...
// Attr: [inline]
inline static DenseArray new_dense_array() {
		return (DenseArray){
			.cap = 8,
			.size = 0,
			.deletes = 0,
...
// Attr: [inline]
inline static u32 DenseArray_push(DenseArray* d, KeyValue kv) {
	if (d->cap == d->size) {
		d->cap += d->cap >> 3;
...
static void map_set(map* m, string key, voidptr value) {
	...
	multi_return_u32_u32 mr_8133 = map_key_to_index(m, key);
	u32 index = mr_8133.arg0;
	u32 meta = mr_8133.arg1;
	multi_return_u32_u32 mr_8167 = map_meta_less(m, index, meta);
	index = mr_8167.arg0;
	meta = mr_8167.arg1;
	while (meta == m->metas[index]) {
...
static void map_rehash(map* m) {
	...
	i < m->key_values.size; i++) {
		...
		multi_return_u32_u32 mr_9336 = map_key_to_index(m, kv.key);
		u32 index = mr_9336.arg0;
		u32 meta = mr_9336.arg1;
		multi_return_u32_u32 mr_9374 = map_meta_less(m, index, meta);
		index = mr_9374.arg0;
		meta = mr_9374.arg1;
...
static void map_cached_rehash(map* m, u32 old_cap) {
	...
	i <= old_cap + old_extra_metas; i += 2) {
		...
		multi_return_u32_u32 mr_9955 = map_meta_less(m, index, meta);
		index = mr_9955.arg0;
		meta = mr_9955.arg1;
...
static voidptr map_get3(map m, string key, voidptr zero) {
	multi_return_u32_u32 mr_10162 = map_key_to_index(&m, key);
	u32 index = mr_10162.arg0;
	u32 meta = mr_10162.arg1;
	multi_return_u32_u32 mr_10196 = map_meta_less(&m, index, meta);
	index = mr_10196.arg0;
	meta = mr_10196.arg1;
	while (meta == m.metas[index]) {
		u32 kv_index = m.metas[index + 1];
...
static bool map_exists(map m, string key) {
	multi_return_u32_u32 mr_10504 = map_key_to_index(&m, key);
	u32 index = mr_10504.arg0;
	u32 meta = mr_10504.arg1;
	multi_return_u32_u32 mr_10538 = map_meta_less(&m, index, meta);
	index = mr_10538.arg0;
	meta = mr_10538.arg1;
	while (meta == m.metas[index]) {
		u32 kv_index = m.metas[index + 1];
...
void map_delete(map* m, string key) {
	multi_return_u32_u32 mr_10821 = map_key_to_index(m, key);
	u32 index = mr_10821.arg0;
	u32 meta = mr_10821.arg1;
	multi_return_u32_u32 mr_10855 = map_meta_less(m, index, meta);
	index = mr_10855.arg0;
	meta = mr_10855.arg1;
	while (meta == m->metas[index]) {
		u32 kv_index = m->metas[index + 1];
...
#ifndef CUSTOM_DEFINE_no_bounds_checking
		if (idx < 0 || idx >= s.len) {
			v_panic(_STR("string index out of range: %"PRId32"\000 / %"PRId32"\000", 3, idx, s.len));
		}
...
#ifndef CUSTOM_DEFINE_no_bounds_checking
		if (idx < 0 || idx >= u.len) {
			v_panic(_STR("string index out of range: %"PRId32"\000 / %"PRId32"\000", 3, idx, u.runes.len));
		}
...
string string_repeat(string s, int count) {
	if (count < 0) {
		v_panic(_STR("string.repeat: count is negative: %"PRId32"\000", 2, count));
	} else if (count == 0) {
		return tos3("");
	} else if (count == 1) {
		return s;
...
static string Color_str(Color c) {
	return _STR("{%"PRId32"\000, %"PRId32"\000, %"PRId32"\000}", 4, c.r, c.g, c.b);
}
...
void _vinit() {
	...
	_const_numbers = new_array_from_c_array(3, 3, sizeof(int), (int[3]){
		1, 2, 3, 
});
	_const_red = (Color){
	.r = 255,
	...
	_const_blue = rgb(0, 0, 255);

}

int main(int ___argc, char** ___argv){
	_vinit();
println(array_int_str(_const_numbers));
println(Color_str(_const_red));
println(Color_str(_const_blue));
	return 0;
}

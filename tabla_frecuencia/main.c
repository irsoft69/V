...
#define V_CURRENT_COMMIT_HASH "12221fb"
#endif
...
struct Tabla {
	f64 max;
	f64 min;
	f64 amplitud_variacion;
	int numero_clases;
	f64 tamanio_clases;
	array_Limites limites_clases;
	array_int frecuencia_abs;
	int total_frec_abs;
	array_f64 frecuencia_rel;
	array_int frec_abs_acumu;
	array_f64 frec_rel_acumu;
};
...
array_f64 _const_data; // inited later
static f64 my_format(f64 f);
static Tabla* Tabla_minimo_valor(Tabla* t);
static Tabla* Tabla_maximo_valor(Tabla* t);
static Tabla* Tabla_amplitud_de_variacion(Tabla* t);
static Tabla* Tabla_numero_de_clases(Tabla* t);
static Tabla* Tabla_tamanio_de_clases(Tabla* t);
static Tabla* Tabla_limite_de_clases(Tabla* t);
static Tabla* Tabla_frecuencia_absoluta(Tabla* t);
static Tabla* Tabla_frecuencia_relativa(Tabla* t);
static Tabla* Tabla_frecuencia_abs_acumulada(Tabla* t);
static Tabla* Tabla_frecuencia_rel_acumulada(Tabla* t);
static Tabla* v_new();
static void print_data();
static void Tabla_print_all(Tabla t);
// variadic structs
struct varg_voidptr {
	int len;
	voidptr args[0];
...
array_int math__digits(int _n, int base) {
	...
	array_int res = __new_array(0, 0, sizeof(int));
	while (n != 0) {
		array_push(&res, &(int[]){ (n % base) * sign });
		n /= base;
	}
	return res;
}
...
static f64 my_format(f64 f) {
	string ft = f64_strlong(f);
	int i = string_index_old(ft, tos3("."));
	if (ft.len < 5) {
		return f;
	}
	return string_f64((string_substr(ft, 0, i + 3)));
}

static Tabla* Tabla_minimo_valor(Tabla* t) {
	t->min = (*(f64*)array_get(_const_data, 0));
	// FOR IN array
	array tmp1 = _const_data;
	for (int tmp2 = 0; tmp2 < tmp1.len; tmp2++) {
		f64 i = ((f64*)tmp1.data)[tmp2];
		if (i < t->min) {
			t->min = i;
		}
	}
...
static Tabla* Tabla_maximo_valor(Tabla* t) {
	f64 m = (*(f64*)array_get(_const_data, 0));
	// FOR IN array
	array tmp1 = _const_data;
	for (int tmp2 = 0; tmp2 < tmp1.len; tmp2++) {
		f64 i = ((f64*)tmp1.data)[tmp2];
...
static Tabla* Tabla_numero_de_clases(Tabla* t) {
	t->numero_clases = math__sqrt(_const_data.len);
	return t;
}
...
static Tabla* Tabla_limite_de_clases(Tabla* t) {
	...
	while (1) {
		...
		if (v == 0) {
			...
			array_push(&t->limites_clases, &(Limites[]){ limite });
		} else {
			limite.i = limite.f;
...
static Tabla* Tabla_frecuencia_absoluta(Tabla* t) {
	int c = 0;
	// FOR IN array
	array tmp1 = t->limites_clases;
	for (int tmp2 = 0; tmp2 < tmp1.len; tmp2++) {
		Limites lim = ((Limites*)tmp1.data)[tmp2];
		c = 0;
		// FOR IN array
		array tmp3 = _const_data;
		for (int tmp4 = 0; tmp4 < tmp3.len; tmp4++) {
			f64 m = ((f64*)tmp3.data)[tmp4];
			if (m >= lim.i && m < lim.f) {
				c++;
			}
		}
		array_push(&t->frecuencia_abs, &(int[]){ c });
		t->total_frec_abs += c;
	}
	return t;
}
...
static Tabla* Tabla_frecuencia_relativa(Tabla* t) {
	f64 vt = 0.0;
	// FOR IN array
	array tmp1 = t->frecuencia_abs;
	for (int tmp2 = 0; tmp2 < tmp1.len; tmp2++) {
		int f = ((int*)tmp1.data)[tmp2];
		vt = ((f64)(f)) / ((f64)(t->total_frec_abs));
		array_push(&t->frecuencia_rel, &(f64[]){ vt });
	}
	return t;
}
...
static Tabla* Tabla_frecuencia_abs_acumulada(Tabla* t) {
	for (int i = 0;
	i < t->frecuencia_abs.len; i++) {
		if (i == 0) {
			array_push(&t->frec_abs_acumu, &(int[]){ (*(int*)array_get(t->frecuencia_abs, i)) });
		} else {
			array_push(&t->frec_abs_acumu, &(int[]){ ((*(int*)array_get(t->frec_abs_acumu, i - 1)) + (*(int*)array_get(t->frecuencia_abs, i))) });
		}
	}
	return t;
}
...
static Tabla* Tabla_frecuencia_rel_acumulada(Tabla* t) {
	for (int i = 0;
	i < t->frecuencia_rel.len; i++) {
		if (i == 0) {
			array_push(&t->frec_rel_acumu, &(f64[]){ (*(f64*)array_get(t->frecuencia_rel, i)) });
		} else {
			array_push(&t->frec_rel_acumu, &(f64[]){ ((*(f64*)array_get(t->frec_rel_acumu, i - 1)) + (*(f64*)array_get(t->frecuencia_rel, i))) });
		}
	}
	return t;
}
...
static Tabla* v_new() {
	return (Tabla*)memdup(&(Tabla){	.max = 0.0,
		.min = 0.0,
		.amplitud_variacion = 0.0,
		.numero_clases = 0,
		.tamanio_clases = 0.0,
		.limites_clases = __new_array(0, 1, sizeof(Limites)),
		.frecuencia_abs = __new_array(0, 1, sizeof(int)),
		.total_frec_abs = 0,
		.frecuencia_rel = __new_array(0, 1, sizeof(f64)),
		.frec_abs_acumu = __new_array(0, 1, sizeof(int)),
		.frec_rel_acumu = __new_array(0, 1, sizeof(f64)),
	}, sizeof(Tabla));
}
...
static void print_data() {
	...
	i < 50; i++) {
		...
		print(_STR("%2.2f\000, ", 2, (*(f64*)array_get(_const_data, i))));
	}
	println(tos3(""));
}
...
static void Tabla_print_all(Tabla t) {
	println(tos3(""));
	println(tos3("Clases		F.Abs	F.Rel	F.Abs.Acu     F.Rel.Acu"));
	println(tos3("------------------------------------------------------"));
	for (int i = 0;
	i < t.limites_clases.len; i++) {
		println(_STR("%2.2f\000 - %2.2f\000	%2"PRId32"\000	%2.2f\000	   %2"PRId32"\000 	  	%2.2f", 6, (*(Limites*)array_get(t.limites_clases, i)).i, (*(Limites*)array_get(t.limites_clases, i)).f, (*(int*)array_get(t.frecuencia_abs, i)), (*(f64*)array_get(t.frecuencia_rel, i)), (*(int*)array_get(t.frec_abs_acumu, i)), (*(f64*)array_get(t.frec_rel_acumu, i))));
	}
	println(tos3(""));
}
...
void _vinit() {
	...
	_const_data = new_array_from_c_array(50, 50, sizeof(f64), (f64[50]){
		1.75, 1.50, 1.68, 1.70, 1.61, 1.65, 1.69, 1.79, 1.78, 1.45, 1.41, 1.43, 1.55, 1.63, 1.60, 1.66, 1.52, 1.66, 1.57, 1.50, 1.56, 1.76, 1.56, 1.58, 1.68, 1.40, 1.56, 1.69, 1.52, 1.63, 1.46, 1.65, 1.48, 1.67, 1.67, 1.78, 1.77, 1.68, 1.74, 1.45, 1.47, 1.57, 1.58, 1.54, 1.76, 1.44, 1.52, 1.61, 1.56, 1.56, 
});
...
int main(int ___argc, char** ___argv){
	_vinit();
Tabla* tf = v_new();
Tabla_minimo_valor(tf);
Tabla_maximo_valor(tf);
Tabla_amplitud_de_variacion(tf);
Tabla_numero_de_clases(tf);
Tabla_tamanio_de_clases(tf);
Tabla_limite_de_clases(tf);
Tabla_frecuencia_absoluta(tf);
Tabla_frecuencia_relativa(tf);
Tabla_frecuencia_abs_acumulada(tf);
Tabla_frecuencia_rel_acumulada(tf);
print_data();
Tabla_print_all(/*rec*/*tf);
	return 0;
}
